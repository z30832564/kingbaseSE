...
public class Clover extends JFrame  
{  
    public Clover()  
    {  
        ...
        ResultSet ret2 = null; 
            ...
            try {
                db1.pst=db1.conn.prepareStatement("select c_name,datediff(year,c_age,sysdate),c_phone,c_salary,c_workplace,meettime,point from marry.candidate,marry.records where c_idcard in (select w_id from marry.records where m_id='"+Login.c_name+"' and  m_to_w>=7 and w_to_m>=7) and marry.candidate.c_idcard=marry.records.w_id and m_id='"+Login.c_name+"'");
            	...
            	if(ret1.next()) {
                    ...
                    while (flag) { 
                        ...
                        row.add(c_name_);
                        String c_age_ = ret1.getString(2); 
                        row.add(c_age_);
                        String c_phone_ = ret1.getString(3); 
                        row.add(c_phone_);
                        String c_salary_ = ret1.getString(4); 
                        row.add(c_salary_);
                        String c_workplace_ = ret1.getString(5); 
                        row.add(c_workplace_);
                        String meettime = ret1.getString(6); 
                        row.add(meettime);
                        String point = ret1.getString(7); 
                        row.add(point);
                        data.add(row);
                       row = new Vector();
                       if(ret1.next()) {
                ...
                if(ret2.next()) {
                    ...
                    while (flag) { 
                       ...
                       row.add(c_name_);
                       String c_age_ = ret2.getString(2); 
                       row.add(c_age_);
                       String c_phone_ = ret2.getString(3); 
                       row.add(c_phone_);
                       String c_salary_ = ret2.getString(4); 
                       row.add(c_salary_);
                       String c_workplace_ = ret2.getString(5); 
                       row.add(c_workplace_);
                       String meettime = ret2.getString(6); 
                       row.add(meettime);
                       String point = ret2.getString(7); 
                       row.add(point);
                       data.add(row);
                       row = new Vector();
                       if(ret2.next()) {
